...
async function fetchIndexShojo() {
    try {
        let products = await fetch("../data/shojo.json");
        products = await products.json();
        console.log(products);
        if (products?.length > 0) {
            document.getElementById("shojoIndex").innerHTML = products.map((each) => templateShojo(each)).join("");
        }
    } catch (error) {
        console.log(error);
    }
