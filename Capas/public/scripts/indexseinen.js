...
async function fetchIndexSeinen() {
    try {
        let products = await fetch("../data/seinen.json");
        products = await products.json();
        console.log(products);
        if (products?.length > 0) {
            document.getElementById("seinenIndex").innerHTML = products.map((each) => templateSeinen(each)).join("");
        }
    } catch (error) {
        console.log(error);
    }
